...
import { technicalSkills as portTechSkills, softSkills as portSoftSkills } from "@/data/portfolio";
...
export default function ProfessionalProfile() {
  ...
  const softSkills = [
    { name: portSoftSkills[0].name, icon: Brain, color: "text-purple-400 border-purple-500/30 bg-purple-500/10" },
    { name: portSoftSkills[1].name, icon: Lightbulb, color: "text-yellow-400 border-yellow-500/30 bg-yellow-500/10" },
    { name: portSoftSkills[2].name, icon: Users, color: "text-emerald-400 border-emerald-500/30 bg-emerald-500/10" },
    { name: portSoftSkills[3].name, icon: Workflow, color: "text-sky-400 border-sky-500/30 bg-sky-500/10" },
    { name: portSoftSkills[4].name, icon: ShieldCheck, color: "text-rose-400 border-rose-500/30 bg-rose-500/10" },
    { name: portSoftSkills[5].name, icon: HeartHandshake, color: "text-amber-400 border-amber-500/30 bg-amber-500/10" },
  ];
            ...
            <div className="p-4 rounded-2xl bg-gradient-to-r from-primary/10 via-purple-500/5 to-transparent border border-primary/20 flex items-start gap-3.5 shadow-sm">
              ...
              <div>
                <strong className="text-foreground font-bold text-sm block mb-0.5">
                  Constant Learner & Growing Engineer
                </strong>
                <p className="text-xs text-muted-foreground leading-relaxed">
                  Continuously exploring modern frameworks, algorithms, and best practices to grow as a well-rounded software engineer.
                </p>
              </div>
            </div>
          </div>
